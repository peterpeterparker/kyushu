...
Provides access to static assets bundled into the worker at build time or read from mounted file system. See the [Static Assets guide](/guides/static-assets) for the instructions to set up the former.
...
The default approach to serve static asset is bundling and serving them to memory. But you might need to read those from a mounted folder, notably if you hit some memory limitation when bundling the worker.
