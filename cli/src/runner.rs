...
pub async fn run(run_config: &RunConfig, worker_config: &WorkerConfig) -> Result<()> {
    ...
    let pre = Arc::new(ServicePre::new(linker.instantiate_pre(&component)?)?);

    let callback = |(pre, config): (Arc<ServicePre<WorkerState>>, WorkerConfig), req| async move {
...
async fn handle_request(
    pre: Arc<ServicePre<WorkerState>>,
    config: WorkerConfig,
